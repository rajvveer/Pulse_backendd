...
async function rankPosts(posts, userId, options = {}) {
    ...
    // One aggregation for all candidates instead of one count query per post
    let velocityMap = null;
    if (includeVelocity && qualityPosts.length > 0) {
        velocityMap = await Like.getBatchLikeVelocities(
            'post',
            qualityPosts.map(p => p._id),
    ...
    const scored = await Promise.all(qualityPosts.map(async post => {
        ...
        if (velocityMap) score += (velocityMap.get(post._id.toString()) || 0) * CONFIG.VELOCITY_WEIGHT;
    ...
    // ...then a single sort, then positional passes. VRR distribution and
    // author diversity define the FINAL order — re-sorting by score after
    // them (the old behavior) silently undid both.
...
async function getTrendingPosts(posts, options = {}) {
    ...
    // Batch velocity lookup — one aggregation instead of one query per post
    const velocityMap = await Like.getBatchLikeVelocities('post', recent.map(p => p._id), CONFIG.VELOCITY_WINDOW_HOURS);
    const wv = recent.map(p => ({
        ...p,
        _velocity: (velocityMap.get(p._id.toString()) || 0) * CONFIG.VELOCITY_WEIGHT,
