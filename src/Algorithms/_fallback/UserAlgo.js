...
function calculateEngagementRate(user) {
    ...
    // Preferred: real recent-engagement fields if a caller supplies them.
    // FIX: these fields do NOT exist on the User schema (only posts/followers/
    // following/likes), so the original always returned 0 — killing niche &
    // trending detection. Fall back to a derivable proxy from the fields that
    // DO exist (avg likes per post / followers) when recent data is absent.
    if (stats.recentLikes !== undefined) {
