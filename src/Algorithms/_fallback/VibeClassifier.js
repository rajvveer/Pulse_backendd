...
class VibeClassifier {
    ...
    static classify(post) {
        ...
        // Strip punctuation from a token for whole-word comparison.
        const clean = (w) => w.replace(/[^a-z0-9']/g, '');
        for (const [vibe, patterns] of Object.entries(VIBE_PATTERNS)) {
            // Strong keywords — match on WORD BOUNDARIES, not raw substrings.
            // The old `text.includes(keyword)` caused false positives like
            // 'goat' in 'scapegoat' or 'zen' in 'frozen'/'dozen'. Negation is
            // now tied to the actual matched occurrence, mirroring the correct
            // weak-keyword logic below.
            for (const keyword of patterns.strong) {
                ...
                if (parts.length === 1) {
                    // Single word: only whole-token matches count.
                    const hits = tokens.filter((t) => clean(t.word) === keyword);
                    if (hits.length) {
                        matched = true;
                        negated = hits.some((t) => t.negated);
                    }
                } else {
                    // Multi-word phrase: require the phrase as a substring, then
                    // read negation from the token that starts the phrase.
                    if (text.includes(keyword)) {
                        matched = true;
                        const startIdx = tokens.findIndex((t) => clean(t.word) === parts[0]);
                        negated = startIdx >= 0 && tokens[startIdx].negated;
                    }
                }
            ...
            // Weak keywords — whole-token match (punctuation-stripped), with
            // per-token negation. Multi-word weak phrases (e.g. 'tired of')
            // fall back to a substring test since they span tokens.
            for (const keyword of patterns.weak) {
                if (keyword.includes(' ')) {
                    if (text.includes(keyword)) scores[vibe] += WEIGHTS.weakKeyword;
                    continue;
                }
                const matchingTokens = tokens.filter(t => clean(t.word) === keyword);
