...
class MoodDetector {
    ...
    async detectMood(userBehavior, options = {}) {
        ...
        // ── 0. Analyze the post's own text/hashtags (strongest signal) ──
        // When a post object is passed (content.text / content.hashtags), its
        // own words are the most direct mood cue and must dominate the ambient
        // time/day biases below. We weight it heavily so a clearly-toned post
        // isn't overridden by what day of the week it happens to be.
        if (userBehavior?.content?.text) {
            this._extractMoodFromText(userBehavior.content.text, 3.0);
        }
        if (Array.isArray(userBehavior?.content?.hashtags)) {
            for (const tag of userBehavior.content.hashtags) {
                if (tag) this._extractMoodFromText(String(tag), 3.0);
            }
        }
     ...
     * @param {string} text
     * @param {number} multiplier - amplifies the contribution of this text
     *        (e.g. a post's own caption counts more than ambient signals).
     *        Defaults to 1.0 so existing callers are unaffected.
     */
    _extractMoodFromText(text, multiplier = 1.0) {
        ...
        for (const [mood, config] of Object.entries(MOOD_KEYWORDS)) {
            ...
            if (config.strong) {
                for (const keyword of config.strong) {
                    if (lower.includes(keyword)) {
                        this.moodScores[mood] += config.weight * 2.0 * multiplier;
                    }
                }
            }
            // Weak keywords
            for (const keyword of config.weak) {
                if (lower.includes(keyword)) {
                    this.moodScores[mood] += config.weight * multiplier;
                }
            }
        }
    }
