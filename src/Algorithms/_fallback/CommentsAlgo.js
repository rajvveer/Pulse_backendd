...
const CONFIG = {
    ...
    // Hard cap on how many comments/replies are scored per ranking pass.
    // Defense-in-depth: even if a caller forgets to paginate, the regex-heavy
    // scoring never runs unbounded on the event loop.
    MAX_RANK: 100,
...
// Repeated-character flood (e.g. "aaaaa"). Hoisted to module scope so it isn't
// recompiled on every isSpammy() call. Note: NOT global — a global regex with
// .test() carries lastIndex state across calls and would skip matches.
const REPEATED_CHAR_PATTERN = new RegExp(`(.)\\1{${CONFIG.SPAM.MAX_REPEATED_CHARS},}`);

// Emoji-flood detection (also hoisted).
...
async function rankComments(comments, options = {}) {
    ...
    // Bound the work: only score up to MAX_RANK items per pass (the regex-heavy
    // quality/spam/toxicity scoring is synchronous CPU on the event loop).
    const toScore = comments.length > CONFIG.MAX_RANK ? comments.slice(0, CONFIG.MAX_RANK) : comments;

    // Score each comment
    const scoredComments = toScore.map(comment => {
