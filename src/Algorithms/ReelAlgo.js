...
const CONFIG = {
    ...
    CREATOR_COLD_START: {
        ...
        MIN_QUALITY: 0.3        // Must still meet quality bar
    },
    SESSION: {
        EARLY_BOOST: 1.3,
        MID_DIP: 0.9,
...
async function rankReels(reels, userId, options = {}) {
    ...
    const scored = await Promise.all(reels.map(async reel => {
        ...
        if (includeVelocity) score += await calculateVelocity(reel._id);
    ...
    // Session pacing (NEW)
    let result = applyReelSessionPacing(scored, sessionDepth);

    // Category diversity (NEW)
    result = enforceCategoryDiversity(result);
